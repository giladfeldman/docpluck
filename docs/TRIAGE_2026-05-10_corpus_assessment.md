...
**Last update:** 2026-05-10 after iter-28 (F3 title rescue) + iter-29 (F2 compound-heading tail merge) landed in iteration-7 session. 221 tests passing.
...
- ✅ **iter-29 / F2 (compound-heading tail) RESOLVED** (this session) — `_merge_compound_heading_tails` reattaches orphaned multi-word JAMA structured-abstract heading tails. Currently curated to `CONCLUSIONS AND RELEVANCE` (the only real-world split observed). jama_open_1 / jama_open_2 fixed: `## CONCLUSIONS\n\nAND RELEVANCE This trial found...` → `## CONCLUSIONS AND RELEVANCE\n\nThis trial found...`.
...
4. ~~**Iter-28 (F3) — title rescue from Abstract.**~~ ✅ DONE this session. 17 of 26 papers now have `# Title`.
5. ~~**Iter-29 (F2 residual) — multi-word heading tokenization.**~~ ✅ DONE this session. `## CONCLUSIONS AND RELEVANCE` no longer split.
6. **Iter-30 (F4) — sidebar Key-Points box detection.** Layout-aware: identify text-block geometrically isolated from the body column (different x-column at same y-range). Preserve as a separate `<aside>` block rather than inlining into the abstract. Affects jama_open_1/2. C2-C3, may need page-bbox awareness similar to F1 structural fix. **NEXT.**
7. **Iter-31 (F6 residual) — banner-strip extension for journal-name-only banners.** korbmacher / chen_2021_jesp / chan_feldman_2025_cogemo / ar_apa_j_jesp_2009_12_010 / social_forces_1 / demography_1 / jmf_1 still show plain-text journal-name banners (`Journal of Experimental Social Psychology`, `Social Forces, 2025, 104, 224–249`, etc.) at line 1 above the rescued `# Title`. Add a curated regex to strip these from the head zone. C1, ~30 min.
8. **Iter-32 (F2 structural) — section-detector misordering for am_sociol_rev_3, amc_1, sci_rep_1.** Library-level; deferred until layout-channel page-bbox awareness lands.
9. **Future (F1 structural)** — page-bbox-aware sentence stitching at page boundaries. Threads page geometry through `extract_pdf` to detect bottom-N% non-body bbox content and skip it BEFORE pdftotext linearizes. C3 / multi-iter. The cheap variant (iter-27) handled the JUNK; this would handle the SENTENCE-SPLIT itself.

After iter-28/29 (this session), iter-30 (Key Points) is the highest-value remaining target on the JAMA papers. iter-31 (extra banner-strip) is C1 and would clean the doc-start of 7 more papers — cheap reader-impact win. Consider doing iter-31 first if iter-30 stalls.
