...
**Last update:** 2026-05-11 after iter-32 (F4 JAMA Key Points sidebar reformat) + iter-33 (extra page-footer / running-header strip patterns) landed in iteration-8 continuation. 245 tests passing. **All 26 papers now open with `# Title` as line 1; JAMA papers now have a clean Key Points blockquote with the CONCLUSIONS sentence stitched.**
...
9. **Iter-34 (F8) — numbered subsection promotion.** `^(\d+\.)\s+[A-Z][a-z]` at start of paragraph → `### N. Title`. Affects chen_2021_jesp (e.g. `1. Hindsight bias`), korbmacher_2022_kruger (e.g. `1.1 Background`, `1.2.1 Underlying mechanisms`), most replication papers. C2, ~1-2 hr. **NEXT** — cheap remaining high-frequency quality issue.
10. **Iter-35 (F2 structural) — section-detector misordering** for am_sociol_rev_3, amc_1, sci_rep_1 (`## Introduction` with affiliations under it). Library-level; deferred until layout-channel page-bbox awareness lands.
11. **Future (F1 structural)** — page-bbox-aware sentence stitching at page boundaries. Threads page geometry through `extract_pdf` to detect bottom-N% non-body bbox content and skip it BEFORE pdftotext linearizes. C3 / multi-iter. The cheap variant (iter-27) handled the JUNK; this would handle the SENTENCE-SPLIT itself.

With iter-31/32/33 landed, **all 26 papers now have `# Title` line 1 + the JAMA papers' Key Points sidebar is clean + 5+ papers have running-headers/copyright stripped from body.** Most remaining work is library-level (Camelot table detection, multi-page table assembly, section-detector reordering). Iter-34 (numbered subsection promotion) is the last cheap high-frequency reader-quality lift.
