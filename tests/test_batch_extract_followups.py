"""
Regression tests for the D3/D5/D6/D7 consumer follow-ups shipped in v1.4.2.

Renamed 2026-09-17: the old filename named a downstream project, and this
repo is PUBLIC. The tests are about batch extraction, not about any one
consumer.
...
from conftest import requires_pdftotext  # noqa: E402
...
class TestExtractToDir:
    ...
    def test_real_pdf_happy_path(self, tmp_path: Path):
        # One paper from the custodian's committed manifest. Not a glob: a
        # directory listing that quietly returned nothing used to skip here, so
        # this test reported success without extracting anything.
        pdfs = [require_corpus_pdf("apa/efendic_2022_affect.pdf")]
        report = extract_to_dir(
            [pdfs[0]],
            out_dir=tmp_path / "out",
            level=NormalizationLevel.academic,
