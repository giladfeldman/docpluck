...
**RC-T foundation: char-level column-detection fallback for tight-kerned tables.** `TABLE_EXTRACTION_VERSION` → `2.4.3`; no `NORMALIZATION_VERSION` / `SECTIONING_VERSION` change. Library-internal capability addition — a foundation for the still-open RC-T Layer-1 table-data recovery.
