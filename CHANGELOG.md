...
telemetry work (committed 2026-09-02 as `eb3e84d` + `9291fc3`), the consult record and the
1.9.60 design lineage that led to it, and the 2026-08-27 stratum (`047b4ca`..`0116edc`,
normalization 1.9.59) that an earlier plan would have tagged alone. The tag target is the
...
> ### ✅ TAG TARGET UPDATED 2026-09-02 — THE RELEASE NOW CONTAINS 1.9.62-1.9.64; THE BLOCK BELOW DESCRIBES THE SUPERSEDED PLAN OF TAGGING AT 1.9.59
>
> The correction below (2026-08-30) was written when the plan of record was to tag v2.4.138
> at `e0b230c`, whose content was NORMALIZATION_VERSION 1.9.59. That plan is superseded: the
> 1.9.60 page gate the finding condemns was REDESIGNED as 1.9.62/63 (first-copy-keep; the
> watermark arm gated on page-coverage >=0.90 + modal-per-page >=2 + value-followed <0.50 —
> sections above), and THAT redesigned gate is what this tag ships. The loss class was
> measured on the shipped form, not inferred from the 1.9.60 numbers: 21-paper HEAD-vs-fix
> sample — 34 furniture lines newly removed, 10 lines recovered, ZERO data lines lost — plus
> the corpus idempotency gate green at the tag content (ratchet 0, 1 passed in 321s) with the
> three formerly non-idempotent papers (ar_royal_society_rsos_140081, ieee_access_5,
> nat_comms_5) probed individually idempotent, and the occurrence-count acceptance table at
> the top of this release. The -4,763-char figure below measured the 1.9.60 gate, which never
> shipped. The tables below saying "v2.4.138 = 1.9.59" describe the superseded tag plan and
> are kept as the dated record of why the release was re-scoped.

> ### ⚠️ SCOPE CORRECTED 2026-08-30 — THIS FINDING IS ABOUT 1.9.60, WHICH HAS NEVER BEEN RELEASED
>
...
> 1.9.59`, **which is what v2.4.138 contains**. The block's own baseline is v2.4.138 itself.
...
> | **1.9.59 — v2.4.138** | **13** at `standard` and `academic` | **11** |
...
> load-bearing fact is that **v2.4.138 behaves identically to what production already
...
> times in both files):
>
> | signal | 1.9.59 (v2.4.138) | 1.9.63 (uncommitted) |
> |---|---|---|
...
>
> v2.4.138 still uses the OLD line-index proxy. The rule the block condemns is not here.
>
> **THE THREE SECTIONS BELOW ARE NOT IN v2.4.138 — THEY, AND THIS BLOCK, DESCRIBE
...
### The 2026-08-27 stratum — normalization 1.9.59 (headed "[2.4.138] - 2026-08-27" before the release was re-scoped)
...
Auditing a consumer letter from 2026-08-09 turned up something the letter did not report and
neither did we: **the standalone-page-number strip was deleting published values.**

`_PAGE_NUMBER_LINE_RE` was one `re.MULTILINE` substitution with no page context —

```python
re.compile(r"^(\f*)[ \t]*\d{1,3}[ \t]*$", re.MULTILINE)
```

— so it deleted **every** line whose entire content was a 1-3 digit integer. pdftotext emits a
narrow numeric table column as **one cell per line**, which is that shape exactly. And it was
silently selective: `-4`, `0.5` and `1000` never matched; `0`, `4` and `999` always did.

**The paper that found it.** `10.1136/bmj-2024-080924` (BMJ), supplementary appendix Table S1 p6,
row *"Number of Advancement Maneuvers (attempt #2)"*:

```
2 (0, 4)      2 (1, 4)      0      (-1.5 to 1.5)      0.999
```

We delivered the interval and the p-value **with no estimate**. That is worse than a wrong number:
a wrong number can be challenged, and a confidence interval attached to nothing cannot even be
attributed to a statistic.

**This is the real content of ESCImate's `2026-08-09/DP-14`**, which both sides had spent twelve
days believing was a *column-major table dump*. It is not. Retrieved from Europe PMC
(`PMC11648086`) and re-run: pdftotext's text layer is **row-major and complete** —
`Duration of First Attempt / 46 (37, 58) / 50 (39, 65) / -4 / (-8.7 to 0.7) / 0.096`. The rows the
consumer saw broken were broken by this rule. (Their *refusal* to reassemble was right, and the
reason they gave — that a wrong pairing is a fabricated row, not a missing one — is the reason we
did not touch pairing either.) The mid-word header break they also reported, `Coefficien` /
`Treatmen t Group`, **is not ours**: the appendix is Word-produced and the header cell wraps
mid-word inside a narrow column, arriving already broken with no hyphen. Rejoining it needs the
layout channel to say the two lines are one cell; that stays open as a table-channel item.

**The fix is structural, never inferential** — it asks what the renderer laid out, not what the
number ought to be. One gate survives: a **pagination run**, three or more consecutive values
ascending by one, each at least a page of text apart.

Two other gates were built and **removed by measurement**, and both deletions are recorded in the
source:

* a **page-margin band** ("first or last non-blank line of the page"). Not idempotent — this step
  runs inside a pipeline whose earlier steps also delete lines, so on a second pass the band
  slides inward and swallows a table cell. It broke `test_normalize_idempotent_jama_open_1` at a
  band of 2 and left three corpus papers non-idempotent at 1. Measured worth: **9 strips in
  1,103.**
* an **affiliation-marker** rule (a bare integer directly above `Department of …`, real on
  `10.15626/MP.2022.3108`). Reverted because removing those three furniture lines costs the
  renderer the paper's entire `## Abstract` heading — a bad trade, and it exposed that
  `test_abstract_zone_affiliation_remnant_real_pdf` had been **passing by accident**: an unrelated
  deletion was pushing the affiliation remnant past its six-line window.

**Measured through the SHIPPED PIPELINE, 26-paper baseline** — and that distinction is the
sharpest lesson in this release. The first implementation split on the form feed and required
`value - page_index` to be constant. It was correct on the raw extraction and **inert where the
rule actually runs**: at the call site `ieee_access_5` arrives with **one form feed for seventeen
pages**, because earlier steps consume the boundaries, and the gate stripped **zero** lines. A
measurement taken on raw text said it was fine. So the run is now found in LINE space —
consecutive values, ascending by one, at least `_PAGINATION_MIN_LINE_GAP` lines apart, which a
table column cannot satisfy because its cells are adjacent.

**1,159 page numbers stripped across the 26 papers; 1,100 bare-integer lines survive to the
output.** The old rule removed roughly 2,330 — both halves. Line count is preserved on every
paper. Site census, same corpus: **1,987 body-interior sites in 26/26 papers, 239 of them in 17
papers beside an interval or a p-value** (`python tools/diag/page_number_strip_blast_radius.py
--baseline`).

**It was also invisible.** The step had no count, no `changes_made` key and no log line — it now
tracks `P0_standalone_page_numbers` / `page_numbers_stripped`. A first draft of the fix recomputed
the margin band *after* each removal, which promoted the next line into the band and erased a table
column of `1 2 3 4 5` from both ends inward; its own test caught that, reading the code did not.

### `changes_made[metric]` overwrote instead of accumulating

Filed by ESCImate 2026-08-21 §4 and verified against our source, not inferred from behaviour.
Three rules write `dropped_minus_signs_recovered` (W0g, W0q, W0h) and three write
`minus_signs_recovered`; the line was a plain assignment, so when two fired on one document only
the last one's delta survived. effectcheck publishes this to users as `upstream_sign_rewrites` and
had to document it as a **lower bound**.

It now accumulates, and a new `changes_made_by_step` gives the per-rule breakdown they asked for
second — **including length-neutral rewrites**, which they did not raise and which is the sharpest
edge of the three: a `2` → `-` glyph substitution has a delta of zero, so the very rules this
metric exists for were the ones most likely to vanish from it entirely.

### `eta` matched inside ordinary English words, and fabricated a statistic

Filed by Scimeto 2026-08-21b §1. `_EFFECT_TYPE_PATTERNS` ended its η² alternative with a bare,
unanchored `eta`, and `_effect_type_for` runs over the whole **caption and footnote** — so a table
captioned *"…Cardiom**eta**bolic Risk Factors"* typed **every unlabeled estimate column in that
table** as η². The reporter measured 19 rows emitted with an `eta2` field, **18 outside η²'s domain
[0, 1]**, against **zero** eta tokens anywhere in the extracted text; the column was headed "mean
change from baseline (95% CI)".

**Worse than filed, in two directions they did not test:** `beta coefficient` and `theta` also
returned `eta_squared` — an unstandardized coefficient relabelled as an effect size — and
`partial eta squared`, a *genuine* partial-η² header, did **not** return the partial type. The
Latin spelling now requires no adjacent letter; the Greek forms need no such guard.

**And the [0, 1] domain guard could not fire on the path that needed it.** It carried
`and eta2_inferred`, restricting it to `_infer_anova_eta2_hint` — the path requiring an F in the
row. When the key came instead from `effect_hint`, the caption-and-footnote vocabulary and the
*weakest* of the three evidence sources, the guard was skipped entirely. The docstring promised
"a stray non-proportion estimate is never mistyped"; the promise held only on the path the guard
happened to be attached to. Removed at both sites — the value is still delivered, as `est`; only
the label is refused.

### Two of our own documents were making false claims

- **`docs/README.md`** — the public README, rendered on GitHub and PyPI — still advertised
  `"p = 484"` → `"p = .484"` and `"p = 0,05"` → `"p = 0.05"`. Both rules were deleted in
  v2.4.129/130. Removed, and replaced with an explicit *"What normalization deliberately does NOT
  do"* table; every row of it was re-measured against the live library before being written.
- **`REQUESTS_FROM_ESCIMATE.md`** still stated two acceptance criteria that contradict shipped,
  deliberate behaviour — *"zero U+FFFD after normalization"* and *"add step
  `A3a_thousands_separator_protect`"*. Superseded banner added; it is a record, not a live contract.

### An inline deleter that no audit could see

`_normalize_text` removes every occurrence of a line that repeats >= 5 times with >= 20-line gaps,
through a bare list comprehension in the function body:

```python
lines = [l for l in lines if l.strip() not in repeated]
```

No `_track`, no `changes_made` key, no log line — and not a named function either, which is why an
audit that wrapped **every module-level function in `normalize.py`**, and separately patched
**`NormalizationReport._track`**, found nothing at all while five lines went missing between two
normalization passes. It took `sys.settrace` over the function body to locate. Rule 0g's blind
spot, stated exactly: instrumentation attracts audit, and its absence repels it.

Now tracked as `P0q_repeated_line_strip` / `repeated_lines_stripped`. On `ieee_access_5` it was
deleting **1,280 characters on pass 1 and 95 more on pass 2**, silently.

**A KNOWN DEFECT IS LEFT OPEN HERE, DELIBERATELY, AND IT IS RECORDED IN THE SOURCE.** The step is
not idempotent, and its own comment predicted that — *"causing idempotence drift when pass 2 has a
shorter input"*. `min(gaps)` is a POSITION metric, so an earlier step removing lines can move a
repeated line across the threshold between passes. On `ieee_access_5` the line it then deletes is
`Performance metric`, a repeated table **column header** that
`_looks_like_running_header_or_footer` correctly refuses; this rule takes it on spacing alone.
`tests/test_normalize_idempotent_real_pdf.py::test_normalize_idempotent_corpus` is **RED on that
...
- `docpluck/sections/types.py` imported `fields` but used `field`, making the whole library
  unimportable in the working tree.
- `tools/diag/page_number_strip_blast_radius.py` is new. It reconfigures stdout to UTF-8, because a
  diagnostic that dies on an un-encodable glyph reports nothing, which reads exactly like a clean
  corpus.

## [2.4.137] - 2026-08-21

### A running header's debris vetoed the heading that followed it — and cost one paper its entire body

Meta Science Arena's `2026-08-21-docpluck-deterministic-gap-analysis.md` reported that on 30 real
PubMed Central papers docpluck has **the best gold recall on the board (0.986)** and nearly the worst
precision (0.673): it loses on *document scope*, not on text fidelity. Their headline finding was
that `extract_sections()` on `PMC13137375` emits a **`keywords` span of 69,849 characters** that
swallows the whole article, and they traced it to the adjacency requirement in
`_synthesize_introduction_if_bloated_front_matter`. That defect is real and reproduced exactly.

**It is not the root cause.** The paper has nine clean top-level headings numbered 1..9 and the
annotator emits a hint for none of 1..8. `1 Introduction: the need for regenerative solutions in
reproductive medicine` **is** matched by the canonical heading regex, and is then rejected, because
rule (b) fails on the colon, rule (c) fails on the subtitle, and rule (a) needs corroboration from
`sections/annotators/text.py::_prior_paragraph_is_sentence_terminated` — which walks back over
space/tab/newline and requires the character underneath to be in `.!?/`. The preceding line is the
Frontiers running header, which pdftotext emits as **form feed + `Abuna et al.` + U+0008 BACKSPACE**.
The period is there. The BACKSPACE stands after it. The walk lands on the BACKSPACE, the guard
declares the prior paragraph unfinished, the heading is discarded — and because
`partition_into_sections` fills every span to the *next* matched heading, `KEYWORDS` inherits the
article.

**We already owned half of this diagnosis.** `normalize.py` has recorded since v1.9.57 — yesterday —
that *"One stray BACKSPACE was enough to make the line unequal to its layout-channel twin and so
unstrippable."* That fix went into the **F0 comparison key only** (`_NONSPACE_CTRL_RE`, used solely
by `_key`). The character stayed in the emitted text, where a second, unrelated rule tripped over it
one day later. Fixing a character in the string you *compare* and not in the string you *ship* is how
one defect becomes two.

### C0 — `_strip_furniture_controls` (NORMALIZATION_VERSION 1.9.58)

A new step, run **before** F0 so the comparison key and the output are one rule rather than two.

**The class is one character wide, and it is measured.** Over the 30 held-out papers, all **33**
U+0008 occurrences carry the identical context `'.'` + BS + newline — trailing furniture — and
**zero** carry the overstrike signature `(x)BS(x)`, so no doubled letter can be manufactured by
removing them. A reviewer predicted exactly that regression; printing every site refuted it.

**The other 16 control characters are NOT stripped**, because printing every site also found what
they actually are: corrupted **content** glyphs, not furniture.

| site | in the corpus | what it is |
|---|---|---|
| `Schri\x02macher` | `PMC13132920` | "Schrittmacher" |
| `No\x04allsanitat` | `PMC13132920` | "Notfall…" |
| `A\x03 -B\x03 helices` | `PMC13134433` | helix designators |

A blanket `[\x00-\x08\x0b\x0e-\x1f]` strip — the obvious fix, and the one two reviewers assumed
we meant — would have destroyed text in two papers. Delete furniture, never data.

Those 49 characters across 16 of 30 papers are now **counted** rather than deleted or ignored, in a
new `NormalizationReport.residual_control_chars`. A known limitation written down without a
measurement beside it is an unpaid debt, not a disclosure; recovering them is W0-family work.

### The guard skips page breaks and control characters (SECTIONING_VERSION 1.2.5)

`_prior_paragraph_is_sentence_terminated`'s walk now covers the form feed and
`_NONSPACE_CTRL_RE`. F0 deliberately **keeps** the page boundary (L-052), so a heading first on a
page had a bare form feed before it and was rejected the same way — a reviewer predicted that the
C0 strip and the pending `layout=` rewiring would otherwise re-break each other, and 2 sites in the
corpus confirm it.

**Skipping is not admitting.** After skipping, the character underneath must still be a real
terminator. That is what keeps the guard's founding case — a pdftotext-column-wrapped CRediT list
ending in `;` — rejected. **Measured blast radius: 39 canonical headings are rejected across the 30
papers and exactly 3 are furniture-blocked** (1 backspace, 2 form feeds). The other 36 — 20 blocked
by a letter (genuine column wrap), 9 by a comma, 6 by `;`/`)`/`:`/a digit — are unchanged. The guard
was mostly right; the fix is three characters wide, not a rewrite.

`tests/test_furniture_control_chars.py` opens with the false-positive gates, and the founding CRediT
case is one of them — **written first, because no test had ever called this helper on the input it
was created for.**

### The numbered-subtitle heading — because the BACKSPACE fix alone did NOT fix the paper

This is the part worth reading, and it nearly shipped as a false number.

The obvious probe after finding the BACKSPACE was: take the normalized text, strip the control
characters, re-section, re-score. It gave a clean, large answer. **The shipped pipeline does not do
that.** C0 runs *before* H0, so once the BACKSPACE is gone H0 can finally match the running-header
line it was always meant to catch — and removes it. That leaves the **keyword list** adjacent to the
heading, and the guard rejects on the `y` of "…stem cell therapy" instead of on the BACKSPACE. The
69,849-character span survived untouched. Same characters, same functions, different order, opposite
result. See LESSONS L-056.

The evidence that actually remains is typographic and narrow: the line carries a **line-initial
section number** and the canonical word is immediately followed by a **colon** —
`1 Introduction: <subtitle>`. `_is_numbered_subtitle_heading` requires **both**, and the near-miss
is why: two lines later the same paper prints `2 Literature search`, which carries the same number
prefix — and `literature` resolves canonically to **references**. A number-prefix-only escape would
have opened a back-matter label in the middle of the article and handed the entire body to a DROP
class. A section word followed by a space is a phrase; a section word followed by a colon is a title
and its subtitle.

**Measured blast radius: across the 30 papers the rule fires exactly once**, and the near-miss it
must exclude is the only other candidate in the corpus.

| `PMC13137375` section | v2.4.136 | v2.4.137 |
|---|---|---|
| `keywords` | **69,849 chars** — the entire article | **100 chars** |
| `introduction` | *absent* | 69,525 chars |

### Arena effect — measured end-to-end on a frozen tree, 30 held-out papers, the arena's own scorer

| variant | v2.4.136 | **v2.4.137** |
|---|---|---|
| flat `normalize_text` (what the arena player emits) | 0.6667 | 0.6668 |
| body-label slice on the F0 text | 0.7681 | **0.7911** |
| oracle `max(flat, body)` | 0.8038 | **0.8140** |
| papers where the body slice scores below flat | 5 | **4** |
| flat gold **recall**, median | 0.986 | **0.9862** |
| papers regressed (flat or body) | — | **0** |
...
`partition_into_sections`' boundary-aware truncation was disabled in v1.6.1 by writing
`_NO_TRUNCATE = set(SectionLabel)` — a set containing **every** label, so the loop's first statement
always `continue`d and ~45 lines of span-splitting code could never execute. Deleted;
`test_sections_boundary_truncation.py` pins the disabled behaviour and passes unchanged.
`sections/boundaries.py::is_section_boundary` survives, tested, and is now **declared** as having no
production call site rather than left to be discovered.
...
**Inline cross-references condemned whole tables.** `_CAPTION_LABEL_RE` was unanchored, so a mid-sentence reference in a footnote or prose cell — `"…al. (2007) in Table 8."`, `"…in Figure 2. We summarized the in-"` — read as an *absorbed caption* and rejected the entire grid. An absorbed caption always begins its cell; a reference has sentence text before it. The pattern is now anchored to `^\s*`, matching the discipline `camelot_extract._CAPTION_ROW_PATTERN` already documents (its comment notes that anchoring is precisely why an inline `see Table 2` does not match). The whitespace copy had simply lost the anchor.

**Standard APA composite value cells scored as "not data".** The clean-data-cell test is what proves a grid is a real data table rather than absorbed prose. It matched a *single* numeric token, so it accepted `2.84`, `.67`, `[0.59, 0.73]`, `(170)`, `<.001` — but rejected every multi-token composite: `2.84 [1.89]` (mean [SD]), `3.47 [1.23] (170)` (mean [SD] (n)), `2.84 ± 1.89`, `0.42***`. A descriptives table built from those cells scored zero clean data rows and was discarded as prose, so its grid was thrown away and the table rendered as a caption-only stub. Replaced by a **numeric-dominance** test (`_cell_is_clean_data`: digit-bearing *and* free of substantive words), which generalises to composites not yet encountered instead of chasing each new shape with another alternation. The prose side is unmoved — a cell carrying real words is still not data, which is what keeps absorbed body text out of the grid.

101-PDF guard-diff: **24 tables recover (up from 20 at v2.4.120), 0 regressed, 0 foreign-caption acceptances.** New this release: `ieee_access_7` Table 5 among them.

**A second quadratic regex — and why the first fix looked complete but wasn't.** v2.4.120 bounded `_CI_UPPER_DROPPED_RE`'s integer part and reported the ReDoS resolved. It was not: `normalize_text` still scaled ~4–5× per doubling. The reason it was missed is instructive — the check was an *absolute wall-clock budget*, which passed alone and failed only under parallel load, so it read as flakiness. Rewritten as a **scaling ratio** (load-independent: contention inflates both measurements together), it failed consistently, turning noise into signal.

Instrumenting every *compiled* pattern's `.sub()` — a module-level sweep misses these, they live inside a list literal — attributed **3.165s of 3.7s** at n=20000 to the Royal Society running-footer pattern in `_WATERMARK_PATTERNS`, whose leading unbounded `\d+` starts a match attempt at every digit of a long numeric run and then fails. Bounding it to `\d{1,6}` (a page number is never longer) is **~1370× faster** on the pattern (4.53s → 0.0033s) and **~50× faster end-to-end** (`normalize_text` 4.85s → 0.044s at n=20000), with scaling now genuinely linear at ~2.0× per doubling and both real footer forms matched identically. A lookahead-anchored variant was tried first and **measured worse** (6.4s vs 3.2s) — it was discarded rather than reasoned about.
...
**A table's own caption no longer condemns its own table.** `TABLE_EXTRACTION_VERSION` → `2.4.9`. This is RC-T cycle 4 (user-approved full RC-T cluster). `_whitespace_grid_is_clean` rejects any cell grid containing a `Table N.` / `Figure N.` caption cell — the signature of a caption-anchored region that over-reached and absorbed a *neighbouring* table's caption. But `detect._region_for_caption` builds every region as `_union(caption_bbox, geom_bbox)`, because the region-driven Camelot pass needs the caption for pairing. So a region **always** contains its own caption by construction, and the guard condemned **every** region grid: `whitespace_cells` returned 0 cells corpus-wide (19 of 19 regions across chan_feldman and maier), and each of those tables silently fell through to the raw_text fallback, which truncates rows. The guard's own comment already said the marker proves the region absorbed "an adjacent table's (or its own) caption" — the "or its own" clause was the bug, hiding in the documentation of the rule it broke.

**The fix is identity-based, not position-based.** The anchoring caption's number is threaded into the guard — from the region spec in `extract_structured` for the Camelot region path, and from `region.caption_match` for the whitespace path — and a caption cell is exempt only when it names **that exact number**, sits within `_OWN_CAPTION_MAX_ROW`, and is the first such match. The exemption is per-**cell** and opt-in: `own_caption_number=None` preserves the strict legacy behaviour for the auto-detect path, whose cells are not caption-anchored. A grid whose own caption is char-fragmented across cells fails to match and is rejected — the safe direction, since a reader-visible missing table beats an invisible wrong one.

**A position-only draft was written and rejected in review.** It exempted simply "the topmost caption in a leading row." Cross-model review (codex, 2026-08-04) found three ways that admits a **wrong** table rather than none, and all three reproduced locally before the rework: a **neighbour's** caption landing in a leading row was blessed as "own" (publishing another table's data under this caption); `(r, c)` sort order picks the **leftmost** caption, which on a side-by-side page is the neighbour's; and exempting the whole **row** let a second `Table N.` / `Figure N.` on that row ride along free — contradicting the code's own comment that a leading `Figure N.` remains evidence of mis-bounding. Each is pinned by a regression test naming the reviewer and date. The standing rule this establishes: when a guard must tolerate a legitimate instance of the very marker it rejects, key the exemption on **identity passed down from the caller that constructed the artifact**, never on position — a coarse positional exemption quietly covers neighbours you never meant to admit.

**What it recovers.** APA-corpus guard-diff: **13 tables recover (812 cells), 0 regressed, 0 foreign-caption acceptances** — ip_feldman Table 1, jdm_.2023.15 Tables 2–3, korbmacher Tables 7/9/10/12/14, and maier Tables 3/8/9/10/11. Every recovered grid's leading caption matches its anchoring caption number, including the colon-style `Table N:` variant. The recovery is not specific to the two papers that motivated the cycle, which is the general-fix bar this project requires.

**What it does NOT fix.** chan_feldman's tables stay unrecovered, and its `xfail(strict)` guard stays armed — with a corrected diagnosis. Its regions genuinely absorb surrounding two-column body prose, so its grids fail the prose-contamination and clean-data-row guards on their own merits; that is the separate region prose-trim cycle. Two of its captions (Tables 5 and 8) are **correctly** rejected even now — those regions really do reach into Table 9's caption at grid row 4, which is the guard's original defence case, still intact. The earlier handoff attributed the corpus-wide zeroing to the prose guard; the reproduction showed the caption guard was the universal cause and the prose issue a narrower, real, still-open one.

**A pairing bug the recovery exposed — and a test that had encoded it.** `test_chan_feldman_table8_ci_upper_signs_recovered` began failing. It read the `2bi` / `2bii` hypothesis rows out of **Table 8** — and passed only because the caption→table pairing was itself wrong. The AI gold (article-finder `reading` view; ground truth is never pdftotext) is unambiguous: Table 8 is *"Control condition (Replication): Intercorrelations with confidence intervals"*, an M / SD / α / ω variable matrix, while *"Table 9. Summary of statistical tests and their interpretation"* is the grid whose rows are `1a, 1b, 2a, 2bi, 2bii`. Cycle 4's exemption corrected that pairing, so the hypothesis rows now resolve to Table 9 with gold-exact values (2bi target `[−0.78, −0.66]`, replication `[−0.78, −0.67]`). The test is re-targeted to select the hypothesis grid **by content** rather than by a hard-coded label, so a future pairing change fails loudly instead of silently re-passing against whichever table happens to be numbered 8. The substantive assertions — the recovered negative upper bounds — are unchanged.

**A ReDoS in `normalize.py`, fixed (unrelated to this cycle, found by it).** `test_regex_no_catastrophic_backtracking` had been failing intermittently in the parallel suite and was recorded in the run-4 handoff as a load-sensitive perf test that "passes alone in 1.43s of a 5s budget." Re-measured **unloaded**, that was no longer true: `normalize_text` on the test's own 10k-digit input took 2.3–6.9s across six consecutive runs, blowing the budget on three of them with no parallel load at all. Profiling put 3.17s of 3.33s in `re.sub`, and a per-pattern sweep isolated `_CI_UPPER_DROPPED_RE`. The first hypothesis — that the unbounded lazy decoration gap `[^\[\]\n]*?` was the driver — was **measured and rejected**: bounding it left the pattern at 1.6s. The actual culprit is the decimal atom `\d*\.\d+`, whose unbounded **integer part** consumes the whole remaining digit run at each of ~n start positions and then backtracks one digit at a time hunting for a `.` — quadratic, ~4.6× per doubling (0.49s at n=10k, 2.27s at n=20k). Bounding the integer part to `\d{0,20}` is ~270× faster (1.68s → 0.006s at n=20k) and matches every real form identically (`0.72`, `.67`, `12.5`, `100.25`). Only this pattern was vulnerable: the other decimal atoms are anchored by a required prefix (`-`, `b =`, `[`) so they never start at each digit, which was verified by measurement rather than assumed. `NORMALIZATION_VERSION` is deliberately **not** bumped — the pattern's matching behaviour is unchanged, only its complexity.

**Verification.** Full suite green: **2184 passed, 193 skipped, 2 xfailed, 0 failed** — including the perf test, whose flakiness the ReDoS fix removed. New tests: 9 for the caption guard (the defect itself, the opt-in default, the three review findings, a repeated-own-caption case, and the pre-existing foreign-caption defences) and 6 for the ReDoS (two perf-budgeted complexity guards plus behaviour-preservation cases), all watched failing against the unfixed code first. The maier arm of `test_b1_whitespace_cells_wiring_live` XPASS-alerted exactly as its `strict=True` marker was designed to and is now a real assert. End-to-end wiring was confirmed live rather than assumed — 6 of 6 guard invocations through the full pipeline with Camelot enabled receive a real caption number, so the parameter is not dead wiring (the failure mode this very test file was created to catch in 2026-05-25).

## [2.4.119] — 2026-08-04

**Tables no longer silently drop their first rows.** `TABLE_EXTRACTION_VERSION` → `2.4.8`, `NORMALIZATION_VERSION` → `1.9.46`. This is the gated RC-T cycle (user-approved). When Camelot recovers no cells, `_extract_table_body_text` linearizes the text after a caption as the table's body. Its `body_start` walk stepped break-to-break (preferring `

`) and inspected only a 40-character window, so a caption that WRAPPED — with the table body serialized column-by-column and no blank line until several rows in — overshot the caption's own sentence terminator and silently discarded the leading rows. chan_feldman Table 3 lost its first four rows (`Sample size 239/794`, `Geographic origin`, `Gender`, `Ethnic group`); Table 1 lost its first hypothesis row. A scientist read a table that was simply missing its opening data.

**The fix (`_caption_tail_body_start`).** A per-LINE walk with three structural rules, each pinned by a corpus case the run-3 prototype regressed on: (1) the terminator test measures the WHOLE physical line from its true start, so a self-terminated caption (`Table 6. Study 2 descriptive statistics.`) ends the walk immediately — the prototype measured only the post-match remainder, missed the terminator, and truncated 799 characters of xiao_2021 Table 6; (2) a blank line ends an unterminated caption; (3) a genuine caption sentence wraps at most four physical lines — beyond that the "tail" being consumed is table CONTENT, so the body starts right after the caption's own line, which keeps amc_1 Table 3's bibliography rows (the prototype truncated 352 characters of them). Two companion guards handle what the corrected walk now reaches: `_skip_leading_nontable_junk` drops a leading figure caption or wrapped body-prose paragraph (bounded, and only for a sentence-terminated run, so a real table's first rows are never eaten), and `_raw_text_is_page_furniture_only` suppresses a fallback that is nothing but a next-page running header plus a page marker — jama_open_1 Table 2 rendered only a journal banner, the article title, `October 27, 2023` and `8/13`. `_line_is_body_prose` gains a bibliography-row exemption (author-initials opener plus a publication year) so a collection table's reference rows are not mistaken for prose.

**Two corruptions the recovered rows exposed.** Restoring dropped content surfaced defects that had been invisible because the data never reached the page — each pre-existing, each verified against a v2.4.118 baseline:
- **`≥` mojibake in table rows.** The S5b cmsy10 recovery was wired into channel 1 (`normalize_text`) only, so plos_med Table 2's remnant-size rows (`≥5–10 mm` …) arrived through the raw_text fallback still reading `U+FFFD`. Now wired into channel 3 per the three-channel glyph discipline, with the document-consensus gate extended to count operators already present in the text (unanimity still required, so a mixed document recovers nothing).
- **An unrecoverable estimate column** (`W0b-est`). The estimate column is separated from its CI column by the SE column, so W0d's proximity window cannot span the gap — efendic Table 3's `21.34` stayed corrupt while its own CI `[-1.58, -1.10]` recovered correctly. Estimates now pair with CIs POSITIONALLY (pdftotext emits adjacent numeric columns as one undivided run, so a run that is an exact multiple of the CI-run length is split and its first block taken), and a rewrite happens only when the corrected reading lands inside that CI while the literal one does not — W0d's containment invariant, resolved by column index instead of distance. The SE column and genuine positive estimates are untouched.

**A recovery rejected before release.** A companion `W0b-col` rule was written to recover a `2`-for-minus CI whose bounds happen to ASCEND (efendic Table 5's `[2.42, 2.69]`), reasoning that the rest of its CI column had already recovered to negatives. The canary audit compared every table value against the gold and found no sign discrepancy — because the gold confirms `[2.42, 2.69]` is a **genuinely positive** interval (the Direction high-vs-low contrast, estimate `2.56`, `p < .001`). The rule would have silently published a sign-flipped effect. It was removed before release and the case is pinned by a regression guard. The standing rule: a sign flip requires per-record evidence (descending bounds, estimate-containment, or a surviving layout glyph) — column membership or neighbour majority is never sufficient. The pre-existing regression pattern was itself over-broad (it matched ascending pairs, asserting the wrong flip into existence) and is now descending-only.

**Verification** (ground truth = AI multimodal read via article-finder `reading` golds — never pdftotext). A 101-PDF `body_start` guard-diff shows **333 captions recovering leading rows and 0 truncations** (the run-3 prototype had 2 regressions). chan_feldman T3's four rows are back and its `xfail(strict)` guard XPASS-alerted exactly as designed — it is now a real assert, joined by new truncation guards pinning the amc_1 and xiao shapes. New regression tests cover the furniture suppression, the FFFD third channel, and both minus recoveries, each with a false-positive battery (a lone page number, a table whose cell is a date, number-dense rows, genuine positive CI columns) — all correctly left alone.

## [2.4.118] — 2026-08-03

**A significance threshold no longer loses its decimal point.** `NORMALIZATION_VERSION` → `1.9.45`. The run-3 Sonnet canary audit found ar_apa rendering `β = -.88, t(87) = 2.01, p < 05 (see Fig. 1)` — a scientist reads a nonsense threshold where the source says `p < .05`. Diagnosis: on the AdvGulliv broken-ToUnicode font family the `.` glyph vanishes from BOTH text channels — pdftotext emits nothing, and the pdfplumber char stream reads `p<05(see` with no dot char at all, while every healthy threshold on the same page carries an explicit `.` char. The same absent-glyph class as the painted-pixel `.245` minus: unlike W0h/W0m there is no layout evidence to gate on.
...
- **Factorial-design notation** — `2 (Between-subject factor--Direction: …) 3 2 (Between-subject factor--…) 3 3 (Within-subject factor--…) mixed-subject design`. Each `) 3 <digit> (` boundary's `3` is a corrupted `×` (the trailing digit is the real factor SIZE). W0k needs LETTER flanks; here the `3` sits between a `)` and a `<digit> (`, so it was never seen.
- **Line-wrapped interaction term** — `the three-way interaction (Direction 3` / `Manipulated Attribute × CMA)`. pdftotext wraps `Direction × Manipulated` across a line break, so the `×` (→`3`) at end-of-line and its right predictor on the next line never formed a single-line pair.

**The fix (W0l — `recover_times_design_notation` + `recover_times_wrapped_interaction`).** Both key on a structural signature, never paper identity. (A) A `) 3 <digit> (` boundary flips to `×` ONLY when it ties to a genuine factor parenthetical: EITHER an immediately-adjacent parenthetical names a factor type (between-/within-/mixed-subjects, repeated-measures), OR both sides are factor-SIZE parens (a bare digit right before the `(` — never `f(x)`, `(range 1)`, `(from level 3)`) AND the chain is closed by a `factorial|…-subjects|design` tail. (B) A physical line ending `<Pred> 3` joins to `×` only when the next line starts with a Title-Case predictor word, "interaction" is in the two-line window, and the head is not a plural count noun. Wired into channel 1 (`normalize_text`) AND channel 3 (`render_pdf_to_markdown` post-process) per the three-channel glyph discipline.

**Verification** (ground truth = AI multimodal read via article-finder `reading` golds — never pdftotext). efendic rendered WITH Camelot: `2 (...) × 2 (...) × 3 (...) mixed-subject design` and `(Direction × Manipulated Attribute × CMA)` now match the gold in all three channels (body prose, flattened caption, `<td>` cells); no stray interaction `3` remains. A 16-case FP battery — a range recode `(min 1) 3 2 (max 5)`, a formula `f(x) 3 2 (y)`, `(from level 3) 3 2 (see note)`, `Scores (bin 1) 3 5 (bin 9) … design later`, `Model 3\n…`, `we ran 3\nstudies`, `interaction between condition and 3\ngroups`, an already-correct `2 (Sex) × 2 (Cond)` for idempotency — changes nothing; a wide raw-text corpus scan fires ONLY on efendic. New real-PDF end-to-end test fails at HEAD (v2.4.115), passes with W0l. This resolves efendic's two ×-as-3 residuals; its remaining findings (two demoted subsection headings) are queued.
...
**The fix (`_demote_parallel_prefix_subsection_headings`), keyed on PARALLEL PREFIX STRUCTURE — not "nearest `##` parent".** A `## ` heading is demoted to `### ` only when a `### ` heading earlier in the same section (scanning back, never crossing a different `## ` body-section boundary) shares a ≥2-word stemmed prefix with it AND then diverges. `## Prevalence Estimates Associations…` demotes because `### Prevalence Estimate Errors` shares the prefix "Prevalence Estimate(s)" and diverges (Errors vs Associations). The obvious "nearest `##` parent is Results → demote" signal is deliberately NOT used — it would wrongly demote the `## Discussion` that also has Results as its nearest `##` above; `## Discussion` shares no prefix with any `### ` sibling, so parallel-prefix keying leaves it alone. A **divergence guard** stops a `### ` that is identical to / a pure prefix of the `## ` title (a duplicate-heading artifact) from demoting the gold-correct `## ` — collabra.90203 renders both the correct `## Extension: Perceived Impact of Donation` and a spurious `### Extension:…` duplicate, and the `## ` is now left untouched.

**Verification** (ground truth = AI multimodal read via article-finder `reading` golds, never pdftotext). ip_feldman: the Results subsection now renders `### ` matching the gold; its `### ` siblings and the following `## Discussion` are unchanged. FP-validated across ~35 rendered corpus papers (20 canary/broad-read + 15 fresh psychology): the ONLY demotion is the ip_feldman target — every other paper's `## ` headings are untouched, and collabra's duplicate `## Extension` is correctly preserved. A HEAD-vs-fix diff confirms only ip_feldman changes. 5 new tests (1 real-PDF + 4 unit-contract incl. the duplicate-title and section-boundary guards). The `WellBeing`→`Well-Being` hyphen drop inside this heading is OCR-tier (raw pdftotext drops the hyphen) and remains won't-fix.
...
**The fix (`_rejoin_wrapped_subsection_heading`, runs FIRST among the isolated-heading promoters).** It rejoins the wrap into one line and emits it already promoted to `### ` — so the level decision is owned by the pass that has the structural evidence, instead of leaving the joined title for the ≤6-word / 5–12-word promoters (which would miss it once joined, or assign the wrong `## ` level). Strict structural signature, keyed on wrap typography, no paper identity: a paragraph-isolated Title-Case head at a clean sentence boundary, GLUED (no blank) to 1–2 heading-ish tail lines, over genuine body prose; the head must "hang" (end in a continuation/function word — "Between", "and", a possessive "Target's", or a colon-titled head like "Challenging and Reframing Misestimation:") OR the joined tail must end in a balanced "(qualifier)"; the head must be SUBSTANTIAL (colon-led title or ≥3 words — so a bare 2-word figure-diagram node-label pair like "Support for"+"Leader" is never merged); and no `Fig. N.`/`Figure N.` caption may sit adjacent (figure node-labels live next to their caption). Joined title capped at ≤115 chars / ≤14 words.

**Verification** (ground truth = AI multimodal read via article-finder `reading` golds, never pdftotext). ip_feldman: all five wrapped headings now render as single `### ` headings matching the gold exactly (`Intensity Estimates Associations with Well-Being (Extension)`, both `Complementary Analysis:` headings, `External Analysis: Suppression Using Target's Analyses`, and the colon-titled `Challenging and Reframing Misestimation: Prevalence and Intensity, Positive and Negative`), with zero stranded tails and zero `## ` over-promotions. An independent Sonnet canary AI-verify confirms all five wrapped-heading defects are gone (ip_feldman's remaining findings — the C3 heading-LEVEL over-promotion and Table 10 loss — are different defect classes, queued as their own cycles). Generalizes beyond the canary: also correctly reunites collabra.90203's gold-confirmed `Associations between Aggregated Feelings and Hypothetical Donations` and a Psychological Science `Contextual Confidence and Postdecision Processing` heading — both previously rendered as plain body text (a real SECTION-BOUNDARY defect). A 16-paper broad-read + canary FALSE-POSITIVE scan produced ZERO prose-merges (the only joins were the intended headings); a HEAD-vs-fix content-loss scan across 18 papers confirms zero body words lost AND that only the 3 papers with genuine wrapped headings changed at all (the other 15 are byte-identical). 5 new tests (2 real-PDF ip_feldman + 3 unit-contract) fail at HEAD and pass with the fix; the previously-shipped 26-paper canary set shows no regression.

**Also fixed (test-consistency debt from the v2.4.105 `SECTIONING_VERSION` 1.2.2→1.2.3 bump, left red until this run's full-suite run surfaced it).** Two stale unit assertions (`test_sections_public_api`, `test_cli_sections`) and three golden snapshots (`tests/golden/sections/*.json`) still expected `1.2.2`; the CLI test now asserts against the imported `SECTIONING_VERSION` so it no longer goes stale on future bumps, and the goldens were regenerated (version field only — no content drift). No library code changed for this part.

## [2.4.107] — 2026-07-03

**Author-affiliation footnote blocks no longer leak into the body.** The cycle-1 canary AI-verify found efendic_2022_affect leaking its author affiliation + corresponding-author block into the MIDDLE of an Introduction paragraph — the block is a page-1 bottom footnote that pdftotext serialised in reading order, splitting a sentence: "…can be far easier--more" │ *Maastricht University … Contributed equally, joint first authors … Gilad Feldman, … Pok Fo Lam Road …* │ "efficient--than weighing…". The front-matter masthead strip only covers the H1→first-`##` zone, so this body-position block was missed.

**The fix (`_strip_body_affiliation_block`, block-level and strict).** It removes a **run of ≥3 consecutive affiliation-shaped lines** (blank-separated tolerated) that begins after the front-matter, reconnecting the split paragraph. A line is "affiliation-shaped" only if it names a university/department/school-of-X, ends in a country, is a `Contributed equally` / corresponding-author / `*` marker, or is a street-address/postcode line — and a **long body sentence (≥11 words ending in a period with several lowercase words) is never counted as affiliation even if it names a university**. Because it fires only on a ≥3-line run, a lone body mention ("The University of X provided seed funding…") is never touched — verified by unit FP tests and a corpus content-loss scan that logs exactly what each paper strips.

**Verification** (ground truth = AI multimodal read via article-finder `reading` golds, never pdftotext). efendic: the affiliation block is gone from the Introduction and the split sentence reconnects ("…can be far easier--more efficient--than weighing…"); the legitimate author-bio university mention later in the paper is preserved. 387 render + masthead + sections + metadata-leak tests pass (incl. 4 new tests — block strip + reconnect, lone-mention-left, below-threshold-left, real-PDF efendic); a logged corpus content-loss scan across 20 Sage/Elsevier papers confirms the strip removes only affiliation footnotes, never body content.

**Milestone: efendic is now the run's first fully-resolved canary** — its glyph cluster (A1 2-for-minus, A2 ×-for-3, A3 SE-sign) and this affiliation leak are all fixed. Remaining canary defects live on other papers (ip_feldman C1/C3 headings + Table 10; plos_med/chandrashekar RC-T table-loss; ar_apa figure-leak + OCR-tier .245).
...
**Root cause (reproduce-before-fix corrected a wrong first hypothesis).** The initial guess was that W0d over-recovered a corrupt `2X.XX` SE cell. Instrumenting the actual render path disproved it: the SE cell arrives genuinely `0.06`, W0d leaves it alone, and the spurious minus is added later by `recover_dropped_minus_via_ci_pairing` (the *dropped*-minus recovery). In the "B | SE | CI" row layout the CI describes the **B** estimate; once B is recovered to `-0.09`, the recovery scans the row for a bare positive whose negation fits the CI and finds the SE `0.06` — `-0.06 ∈ [-0.21, 0.04]` while `0.06 ∉` — so it "recovers" a minus onto the SE. But the SE legitimately falls outside the estimate's CI (a standard error is not bounded by `[lo, hi]`); it is a different column from the estimate the CI pairs with.
...
**'×'-for-'3' glyph recovery for interaction-term table cells — efendic's regression predictors now read "Direction × manipulated attribute", not "Direction 3 manipulated attribute".** `NORMALIZATION_VERSION` → `1.9.38`. The cycle-1 AI-gold canary re-verify surfaced a second glyph corruption on efendic_2022_affect: the multiplication sign `×` (the interaction operator in every predictor name across regression Tables 2–5) decodes as the literal digit `3` — `PMA × direction` reads `PMA 3 direction`, `Pleasure × Arousal × CMA` reads `Pleasure 3 Arousal 3 CMA` — the same broken-ToUnicode AdvPS… subset font that maps U+2212 minus to `2` (v2.4.102) and `<` to `\` (v2.4.39) also maps `×` to `3` (`pdffonts`: uni:no).
...
**Cycle 1 (run 9, harness-gated) — Adobe-Symbol-font glyphs surfaced as Private-Use-Area codepoints (glyph, S1).** Some PDF/DOCX producers embed the Adobe "Symbol" font with no ToUnicode CMap, so pdftotext / mammoth surface each Symbol glyph as a Private-Use codepoint U+F000+<symbol-byte> — the standardized regression coefficient β reads as U+F062, the χ of a χ² test statistic as U+F063, a bullet • as U+F0B7. A PUA codepoint carries no Unicode identity; it is purely a font-encoding artifact and is never legitimate in extracted academic text. The verification harness's Tier-D `glyph` check flagged it on a manuscript DOCX in the `docxtests` source (β ×2) and `escicheck Xiao-etal-2024 Monin&Miller` (χ ×2, bullet ×2).
