...
- **A ZERO IS A CLAIM ABOUT THE INSTRUMENT UNTIL YOU PROVE OTHERWISE.** (User directive 2026-08-15.) `tools/diag/repair_site_scan.py` reported **0 sites for every W0 glyph rule**, which reads as "these rules never fire" and means nothing of the sort: its corpus is the 26-paper render baseline, and that baseline contains **none of the papers those rules were built for**. A denominator that excludes every known positive cannot speak to prevalence. This is the same trap as the retired locale detector ("396 English papers -> 0 European" measured the INSTRUMENT) and as the `A6` superscript check, where 0 hits in 146 papers turned out to mean pdftotext flattens superscript codepoints before any rule sees them. **Before reporting a zero: run the detector against a KNOWN POSITIVE and show it fires.** If you cannot produce a known positive, say the measurement is unbounded rather than clean.
  ...
  **A FOURTH instance, found 2026-09-21, and it names a cause the other three do not: WHICH COPY OF THE LIBRARY DID THE SCAN IMPORT?** Python puts a SCRIPT FILE's OWN directory on `sys.path[0]`, and `tools/diag/` is not the repo root, so until 2026-09-02 **fourteen** scans there imported whatever docpluck was INSTALLED rather than this tree. **Not "never the cwd" — that over-reach was corrected 2026-09-22 after a peer session refuted it, and it had been sitting in four files including this one.** Measured four-sided from the repo root: `python -c` (`sys.path[0] == ''`) and `python -m` both resolve the WORKING TREE; a script file, whether inside the repo or outside it, resolves SITE-PACKAGES. The wrong form mispredicts `python -m pytest`, which does get the tree. `repair_site_scan.py` attributes sites by the step name the library reports back — so its `A3d` **0 sites in 297 English papers** was printed by a library that contained no `A3d` at all, and was a zero *by construction*. Four of the fourteen were missed by an earlier census because they DID carry a `sys.path.insert` — of their own directory, which is already `sys.path[0]` and reaches nothing: a guard that looks applied and is not. **So the known-positive control must be run against the same copy the scan imports, named by PATH and not by `__version__`** — this tree reports `2.4.144` while sitting in no tag and no commit, and the installed copy is routinely several releases stale. The resolution is now pinned by `tests/test_harness_scripts_import_the_working_tree.py`, which runs every script for real and asserts the resolved path.
...
**2026-09-21: the 297-paper half of that precedent is UNVERIFIABLE** — the scan behind it
imported an installed copy of the library rather than this tree, and the state it ran against
cannot be reconstructed (see the provenance note under the EU-numeric rule above). The
*principle* — correct-over-total, not prevalence — is untouched, and the `W0k` figure is
unaffected. Do not carry the 297-paper counts into a new argument without re-measuring them.
